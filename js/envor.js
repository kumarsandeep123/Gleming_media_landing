(function($) {
    ...
    $('.envor-about-widget p.contacts').first().css({
    ...
    $('.envor-estate-form-item').each(function() {
        ...
        if ($list.find('> p').length > 8) $list.css({
            'overflow': 'scroll',
            'overflow-x': 'hidden'
        });
    ...
    $('.envor-top-bar').find('ul.social-btns li').each(function() {
        ...
        $this.hover(function() {
            $(this).find('a.regular').stop(true, true).animate({
                'top': '40px'
            }, 200);
            $(this).find('a.hover').stop(true, true).animate({
                'top': '0px'
            }, 200);
        }, function() {
            $(this).find('a.regular').stop(true, true).animate({
    ...
    $('.envor-top-bar .shopping-cart').each(function() {
        ...
        $(this).hover(function() {
            $(this).find(' > span i.regular').stop(true, true).animate({
                'top': '40px'
            }, 200);
            $(this).find(' > span i.hover').stop(true, true).animate({
                'top': '0px'
            }, 200);
    ...
    function envorMegaMenu() {
        if ($('.envor-header-1 .envor-mega').length != 0) {
            ...
            $menu.slice(megaMenuIndex + 1).each(function() {
                rightOffset = rightOffset + $(this).width() + parseInt($(this).css('padding-left')) + parseInt($(this).css('padding-right')) + parseInt($(this).css('border-left-width')) + parseInt($(this).css('border-right-width'));
            });
        ...
        if ($('.envor-header-2 .envor-mega').length != 0) {
            ...
            $menu.slice(0, megaMenuIndex).each(function() {
                leftOffset = leftOffset + $(this).width() + parseInt($(this).css('padding-left')) + parseInt($(this).css('padding-right')) + parseInt($(this).css('border-left-width')) + parseInt($(this).css('border-right-width'));
            });
    ...
    var $mobileMenu = $('.envor-mobile-menu nav'),
        isMobileMenuShown = 1;
    ...
    $mobileMenu.find('li').each(function() {
        var $ul = $(this).find('> ul');
        if ($ul.length > 0) $('<i class="glyphicon glyphicon-plus-sign"></i>').appendTo($(this));
    ...
    $('.envor-project, .envor-property-1, .envor-product-1, .envor-post-preview, .envor-post').hover(function() {
        $(this).find('figure figcaption').stop(true, true).fadeIn(200).find('i').stop(true, true).animate({
            'top': '50%'
        }, 200);
    }, function() {
        $(this).find('figure figcaption').find('i').stop(true, true).animate({
            'top': '0'
        }, 200).end().stop(true, true).fadeOut(200);
    });
    $('a.colorbox').colorbox();
    var envorLogoH = 0;
    ...
    function envorPricing2() {
        $('.envor-pricing-2').each(function() {
            ...
            $elems.each(function() {
                ...
                if ($(this).hasClass('envor-pricing-2-item-featured')) index = $(this).index();
    ...
    $('.envor-domain-price').each(function() {;
        ...
        $(this).css('position', 'relative').stop(true, true).animate({
            'margin-top': '-5px'
        }, 200);
    }, function() {
        $(this).css('position', 'relative').stop(true, true).animate({
    ...
    if ($(window).height() < 900) $('.envor-content-404 .envor-soc-buttons-list').css('position', 'relative').css('margin-top', '50px');
    ...
    $('.envor-settings i.fa-cog').click(function() {
        if (showSettings) {
            $(this).parents('.envor-settings').animate({
                'left': '-240px'
            });
            showSettings = 0;
        } else {
    ...
    $(window).resize(function() {
        ...
        envorAnimateSkill();
        if ($(window).height() < 900) $('.envor-content-404 .envor-soc-buttons-list').css('position', 'relative');
        $('.envor-projects-listing').each(function() {
            var $this = $(this),
                offset = parseInt($this.find('.envor-listing-item').css('padding-left')),
                w = parseInt($this.parents('[class*="col-"]').width()),
                h, $elems = $this.find('.envor-listing-item');
            $this.css({
                'margin-left': -offset,
                'width': w + offset
            });
